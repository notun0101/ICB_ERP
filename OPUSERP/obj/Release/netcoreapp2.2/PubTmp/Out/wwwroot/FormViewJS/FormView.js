$(document).ready(function () {
	console.log("Formview.js Loaded")
})
...
function ajaxLoadComputerLiteracy(response) {
	...
	$.each(response, function (i, item) {
		option += `<tr>
							...
							<td>${item.remarks}</td>
							<td>
								<a class="btn btn-success" onclick="EditComputer(${item.Id}, '${item.subjectId}','${item.competencyLevel}','${item.training}','${item.diploma}','${item.remarks}')" href="#heading33"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;
...
function ajaxLoadLanguageInfo(response) {
    ...
    $.each(response, function (i, item) {
        option += `<tr>
						...
						<td>
                                <i class="fa ${item.reading} == "reading" ? "fa-check-circle" : "fa-times")"></i>
                        </td>
						<td>
                              <i class="fa ${item.writing} == "writing" ? "fa-check-circle" : "fa-times")"></i>
                        </td>
						<td>
                              <i class="fa ${item.speaking} == "speaking" ? "fa-check-circle" : "fa-times")"></i>
                        </td>
                          <td>
                              <i class="fa ${item.listening} == "listening" ? "fa-check-circle" : "fa-times")"></i>
                        </td>
...
function ajaxLoadAccountsInfo(response) {
    ...
    $.each(response, function (i, item) {
        option += `<tr>
						<td>${ii}</td>
						<td>${item?.walletType?.walletTypeName}</td>
						<td>${item.accountNumber}</td>
						<td>
							<a class="btn btn-success" onclick="EditAccounts(${item.Id},'${item.bankId}','${item.branchName}','${item.accountNumber}', '${item.walletTypeId}', '${item.walletNumber}')" href="#heading22"><i class="fa fa-edit"></i></a>
...
function ajaxLoadAwardInfo(response) {
    ...
    $.each(response, function (i, item) {
        option += `<tr>
							...
							<td>${item.purpose}</td>
							<td>${formatDatefix(item.awardDate)}</td>
...
function ajaxLoadTrainingInfo(response) {
    ...
    $.each(response, function (i, item) {
        ...
        option += `<tr>
						...
						<td>${item.trainingInstitute?.trainingInstituteName}</td>
						<td>${formatDatefix(item.fromDate)}</td>
						<td>${formatDatefix(item.toDate)}</td>
						<td>${item.remarks}</td>
						<td>
							<a class="btn btn-success" onclick="EditTraining(${item.Id},'${item.trainingTitle}', ${item.trainingCategoryId} , '${formatDatefix(item.fromDate)}', '${formatDatefix(item.toDate)}', ${item.trainingInstituteId}, '${item.remarks}')" href="#collapse11"><i class="fa fa-edit"></i></a>
