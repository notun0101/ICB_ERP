...
namespace OPUSERP.Areas.Accounting.Controllers
{
    ...
    public class JournalVoucherController : Controller
    {
        ...
        public JournalVoucherController(IHostingEnvironment hostingEnvironment, IUserInfoes userInfo, ICostCentreService costCentreService, ILedgerService ledgerService, IAttachmentCommentService attachmentCommentService, IVoucherService voucherService, ISalaryService salaryService, IERPCompanyService eRPCompanyService, IConverter converter, ISignatureService signatureService)
        {
        ...
        [HttpGet]
        public async Task<IActionResult> JournalVoucherList()
        {

            var model = new VoucherMasterViewModel
            {
                GetVoucherMasters = await voucherService.GetvoucherMasterDetailsByTypeId(7),
        ...
        [AllowAnonymous]
        public IActionResult JournalVoucherPdfAction(int MasterId)
        {

            string scheme = Request.Scheme;
            var host = Request.Host;

            string url = scheme + "://" + host + "/Accounting/JournalVoucher/JournalVoucherPDF?MasterId=" + MasterId;
        ...
        [AllowAnonymous]
        [HttpGet]
        public async Task<IActionResult> JournalVoucherPDF(int MasterId)
        {
            ...
            var model = new VoucherMasterViewModel
            {
                VoucherMaster = await voucherService.GetGetvoucherMasterByJournalId(MasterId),
        ...
        public async Task<ActionResult> SaveComment([FromForm] CommentsViewModel model)
        {
            try
            {
                ...
                return RedirectToAction("JournalDetails", "JournalVoucher", new { id = model.actionTypeId, Area = "Accounting" });
        ...
        public async Task<IActionResult> DeleteComments(int Id, int actionId)
        {
            ...
            return RedirectToAction("JournalDetails", "JournalVoucher", new { id = actionId, Area = "Accounting" });
        }
        [HttpPost]
        ...
        [Route("/api/global/GetJournalVoucherInformation/{id}")]
        public async Task<ActionResult<System.Object>> GetJournalVoucherInformation(int id)
        {
            var master = await voucherService.GetGetvoucherMasterByJournalId(id);
            ...
            return Ok(new { master, ddetails, details, prinAccount,costdetails });
        }
    }
}