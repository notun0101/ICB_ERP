...
namespace OPUSERP.Areas.HospitalManagement.Controllers
{
  ...
  ////      }
        ...
        // POST: Info/DoctorCreate
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DoctorCreate([FromForm] EmployeeInfoViewModel model)
        {
            //return Json(model);
            ApplicationUser user = await _userManager.GetUserAsync(HttpContext.User);
        ...
        // GET: Info/AllDoctorList
        public async Task<IActionResult> AllDoctorList()
        {
            ApplicationUser user = await _userManager.GetUserAsync(HttpContext.User);
