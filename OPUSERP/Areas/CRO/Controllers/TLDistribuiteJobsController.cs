...
namespace OPUSERP.Areas.CRO.Controllers
{
    ...
    public class TLDistribuiteJobsController : Controller
    {
        ...
        public TLDistribuiteJobsController(IHostingEnvironment hostingEnvironment, IDistributeJobService distributeJobService, IUserInfoes userInfo, IAttachmentCommentService attachmentCommentService, IConverter converter)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            {
                operationMasters = await distributeJobService.GetOperationMasterByTLDistribuiteJob(empCode),
            ...
            if (model.opMasterId != null)
            {
                if (model.opMasterId.Length > 0)
                {
                    ...
                    for (int i = 0; i < model.opMasterId.Length; i++)
                    {
                        ...
                        await distributeJobService.UpdateOperationMasterTLDistribuiteJob(Convert.ToInt32(model.opMasterId[i]), jobStatusId, declaration, model.assignDate, model.remarks[i],model.assignTo);
